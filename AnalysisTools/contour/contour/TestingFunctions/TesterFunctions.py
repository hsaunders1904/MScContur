...
def Var_mu_comb_proto(b_count,s_count,db_count, ds_count):
#Construct the inverse variance matrix from expected vals of the 2nd derivatives of the Log Likelihood
    Var_matrix_inv=np.zeros([(len(b_count)+len(s_count)+1),(len(b_count)+len(s_count)+1)])
#add exception handling if s=/=b
  #loop over all counts
    for i in range(0, len(b_count)+len(s_count)):
      ...
      #mu b_i/b_i mu
        if i < len(b_count):
            Var_matrix_inv[0,0] += s_count[i]**2*b_count[i]/(mu_test*s_count[i]+b_count[i])**2
            Var_matrix_inv[i+1,0]=Var_matrix_inv[0,i+1] = s_count[i]*b_count[i]/(mu_test*s_count[i]+b_count[i])**2
            if db_count[i]**2 > 0.0:
                Var_matrix_inv[i+1,i+1]=b_count[i]/(mu_test*s_count[i]+b_count[i])**2 + 1/db_count[i]**2
            else:
                Var_matrix_inv[i+1,i+1]=b_count[i]/(mu_test*s_count[i]+b_count[i])**2
        if i>=(len(b_count)):
            Var_matrix_inv[i+1,0]=Var_matrix_inv[0,i+1] = (b_count[i-len(b_count)]*mu_test*s_count[i-len(b_count)])/(mu_test*s_count[i-len(b_count)]+b_count[i-len(b_count)])**2
            if ds_count[i-len(b_count)]**2 >0.0:
                Var_matrix_inv[i+1,i+1]=(mu_test**2)*b_count[i-len(b_count)]/(mu_test*s_count[i-len(b_count)]+b_count[i-len(b_count)])**2 + 1/ds_count[i-len(b_count)]**2
            else:
                Var_matrix_inv[i+1,i+1]=(mu_test**2)*b_count[i-len(b_count)]/(mu_test*s_count[i-len(b_count)]+b_count[i-len(b_count)])**2
        if i < len(s_count):
            Var_matrix_inv[len(b_count)+1+i,i+1] = Var_matrix_inv[i+1,len(b_count)+1+i] = mu_test*b_count[i-len(b_count)]/(mu_test*s_count[i]+b_count[i])**2
    if np.linalg.det(Var_matrix_inv) == 0:
        Var_matrix = np.zeros([(len(b_count)+1),(len(b_count)+1)])
  #Invert and return it
    else:
