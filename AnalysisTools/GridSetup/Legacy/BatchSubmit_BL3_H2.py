...
for i in range(150,1001,100):
    for j in range(5,61,5):
        i = int(i)
        j = int(j)
        modelpath = 'mH2_'+ str(i) + '_Sa_' + str(j)
        copytree(pwd + '/GridPack',modelpath)
        #mkdir_p(str(modelpath))
        HerwigString = ''
        HC=open('HerwigCommandH2', 'r')

        HerwigString += 'read FRModel.model \n'
        HerwigString += 'set /Herwig/FRModel/Particles/H2:NominalMass ' + str(i) + '.*GeV \n'
        HerwigString += 'set /Herwig/FRModel/FRModel:Sa ' + str(j/100.0) + '\n'
