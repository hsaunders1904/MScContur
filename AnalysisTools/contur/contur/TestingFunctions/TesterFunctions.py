...
def Min_function(x, n_obs, b_obs, s_obs, db, kev):
    ...
    # Here mu_hat is passed and included into the functional form of the first derivatives, it's set to 1 in the
    # current assumption
    # so doesn't appear
    ...
    # TODO hit some numerical stability here (db**2 too large) handle properly later
...
def Covar_Matrix(b_count, s_count, db_count, kev):
    ...
    for i in range(0, len(b_count) + len(s_count)):
        ...
        # Major restructing here, these min_finds can be factored out to the main conf level function as we only need to
        # work these out once
        # The big change in the code needed here is to sort out the form of the second derivatives, which are all
        # correct in the paper
        # I used a cancellation of n and mu.s+b here which was fine before but as we separate these two counts, the form
        # of these needs to be updated to reflect as in the paper
        ...
        if i < (len(b_count)):
            ...
            res = Min_find(b_count[i - len(b_count)], b_count[i - len(b_count)], s_count[i - len(b_count)],
                           db_count[i - len(b_count)], kev[i - len(b_count)]).x
        ...
        if i < len(b_count):
            ...
            if fabs(db_count[i]) <= 1e100 and db_count[i] ** 2 > 0.0:
                Var_matrix_inv[i + 1, i + 1] = 1 / (mu_test * s_hat_hat + b_hat_hat) + 1 / db_count[i] ** 2
            else:
                Var_matrix_inv[i + 1, i + 1] = 1 / (mu_test * s_hat_hat + b_hat_hat)
        if i >= (len(b_count)):
            ##mu s
            Var_matrix_inv[i + 1, 0] = Var_matrix_inv[0, i + 1] = (mu_test * s_hat_hat) / (
                    mu_test * s_hat_hat + b_hat_hat)
            ##s s
            if s_hat_hat > 0.0:
                Var_matrix_inv[i + 1, i + 1] = (mu_test ** 2) / (mu_test * s_hat_hat + b_hat_hat) + kev[
                    i - len(b_count)] / (s_hat_hat ** 2)

            else:
                Var_matrix_inv[i + 1, i + 1] = (mu_test ** 2) / (mu_test * s_hat_hat + b_hat_hat)
        if i < len(s_count):
            ## b s
            Var_matrix_inv[len(b_count) + 1 + i, i + 1] = Var_matrix_inv[i + 1, len(b_count) + 1 + i] = mu_test / (
                    mu_test * s_hat_hat + b_hat_hat)
...
def qMu_Asimov(mu_test, bCount, sCount, db):
    ...
    for i in range(0, len(bCount)):
        ...
        if N_exp_b_hat * N_exp_b_hat_hat == 0:
            ...
            result += -2. * (bCount[i] * log(N_exp_b_hat_hat / N_exp_b_hat) + N_exp_b_hat - N_exp_b_hat_hat + gauss_exp(
                bCount[i], b_hat, db[i]) - gauss_exp(bCount[i], b_hat_hat, db[i]))
    return result


def chisquare(background, signal, measurement, bgerr, sigerr, measerr, isRatio, mu_test):
    # returns the Chi2 based on comparing the data (measurement) to a possible signal (sigCount - scaled by a mu_test.
    # Could be zero.) and a background prediction (background - could be identical to measurement) and the relevant
    # uncertainties

    # if an element is flagged as a ratio, the measurement is compared to signal or background depending on mu_test
    # if an element is not flagged as a ratio, the measurement is compared to mu_test*signal+background depending on mu
    # test
    ...
    if test == 'LL':
        ...
        if varMat <= 0:
            ...
            if 0 < q_mu <= (mu_test ** 2) / (varMat):
                ...
                # 1/(1-p_b) = 1/ (1-0.5) = 2
                # To evaluate this properly, the procedure above needs to be run again, but under the null hypothesis,
                # i.e where mu_test is 0
        ...
        # signal + background
        chisq_p_sb = spstat.norm.sf(
            np.sqrt(chisquare(background, signal, measurement, bgErr, sgErr, measErr, isRatio, 1)))
        # background only
        chisq_p_b = spstat.norm.sf(
            np.sqrt(chisquare(background, signal, measurement, bgErr, sgErr, measErr, isRatio, 0)))
