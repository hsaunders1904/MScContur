...
def qMu_MC(n, b_til):
    ...
    if N_exp_b_hat * N_exp_b_hat_hat == 0:
        ...
        result = -2. * (n * log(N_exp_b_hat_hat / N_exp_b_hat) + N_exp_b_hat - N_exp_b_hat_hat + tf.gauss_exp(b_til,
                                                                                                              b_hat) - tf.gauss_exp(
            b_til, b_hat_hat))
    return result
...
def qMu_obs_MC(n_obs, b0_in, db_in, s0_in, mu_test_in):
    # build the observed test statistic then run MC pseudoexperiments to estimate the pdf from the number of events
    # exceeding the observed mu
