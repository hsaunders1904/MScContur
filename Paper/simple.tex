...
In this paper we make extensive use of these capabilities within Herwig~\cite{Bahr:2008pv} \todo{DG: Herwig 7 cite}.\todo{MK: should we always use special fonts like small caps for computer tools (Herwig, Feynrules, rivet etc? JMB: I think each has their own convention so we should follow that?}
\item
...
Starting with the measurements discussed in Section~\ref{sec:measurements} \todo{DG: check list is consistent with the final runs; show table of rivet IDs for quick reference here}, 
...
mediator decays to dark matter rather than back to quarks. \todo{MK: maybe we should show a couple of example Feynman diagrams at some point? JMB: I think that would be very good 
