...
measurements at particle colliders, is presented. The method, `Constraints On New Theories Using \rivet' - \Contur \ - exploits  \todo{MK: how do you typeset hyphen: -, --, or ---? }
...
%
\begin{flushright}IPPP-16-52, MCNET-16-nn, TTK-16-22\end{flushright}
\end{abstract}
...
and detector simulation via the RECAST~\cite{Cranmer:2010hk} project.}\todo{MK: are footnotes referenced before or after the period: \ldots blabla.\footnote{This is important.} or 
\ldots blabla\footnote{This is also important.}? (I think I changed some to the former, but maybe that's not what you do in English?}
...
The most relevant production and decay channels for the mediator are illustrated in Fig.~\ref{fig:feyndia}.\todo{MK: These could be a little smaller, but it is fine with me to leave them as is.} 
...
routines were modified as a part of this work, and are all fixed in release 2.5.X.}\todo{MK: Rivet version number not complete.}
...
%\todo{DY: I haven't included 4jet 8tev in what we currently display, it had pretty much no sensitivity to the model and was a whole additional set of event generation. Cut this out? I (JMB) think since we tried it and checked, it's best to leave this wording in and adding it to the table.}.
\item
...
%\todo{DY: I am unsure on whether there is a guarantee of no statistical overlap of the photon + jet and inclusive photon, I mark as separate catagories for now but is this safe? has almost no impact either way as one is 37pb-1 so gives no stat power anyway. JMB: there will be overlap if they are measured in the same sample.}
where available. We also made a new \rivet routine for the CMS photon-plus jet measurement \cite{Chatrchyan:2013mwa}. 
%\todo{DY: Ref for unvalidated CMS gamma + jet we include?}
\item
...
\rivet ID (Inspire) and reference & \rivet description  & \Contur Category \\
\hline
ATLAS\_2014\_I1325553 \cite{Aad:2014vwa} & Measurement of the inclusive jet cross-section at 7 TeV & ATLAS 7 Jets \\
ATLAS\_2014\_I1268975 \cite{Aad:2013tea} & High-mass dijet cross section & ATLAS 7 Jets \\
ATLAS\_2014\_I1326641 \cite{Aad:2014rma} & 3-jet cross section with 7 TeV data & ATLAS 7 Jets \\
ATLAS\_2014\_I1307243 \cite{Aad:2014pua} & Measurements of jet vetoes and azimuthal decorrelations in dijet events produced in pp collisions at $\sqrt{s}$ = 7 TeV using the ATLAS detector & ATLAS 7 Jets \\
CMS\_2014\_I1298810 \cite{Chatrchyan:2014gia} & Ratios of jet pT spectra, which relate to the ratios of inclusive, differential jet cross sections & CMS 7 Jets \\
ATLAS\_2015\_I1394679 \cite{Aad:2015nda}  & Multijets at 8 TeV  & ATLAS 8 Jets \\

%%####ZJets
ATLAS\_2013\_I1230812 \cite{Aad:2014qxa} & $Z$ + jets in $pp$ at 7 TeV & ATLAS 7  Z Jets \\
CMS\_2015\_I1310737 \cite{Khachatryan:2014zya} & Jet multiplicity and differential cross-sections of $Z$+jets events in $pp$ at $\sqrt{s} = 7$ TeV & CMS 7 Z Jets \\

%%####WJets
CMS\_2014\_I1303894 \cite{Khachatryan:2014uva} & Differential cross-section of $W$ bosons + jets in $pp$ collisions at $\sqrt{s}=7$ TeV & CMS 7 W Jets \\
ATLAS\_2014\_I1319490 \cite{Aad:2013ysa} & W + jets & ATLAS W jets \\

%%####Gamma(+jets)
ATLAS\_2013\_I1263495 \cite{Aad:2013zba} & Inclusive isolated prompt photon analysis with 2011 LHC data & ATLAS 7 Photon Jet \\
ATLAS\_2012\_I1093738 \cite{ATLAS:2012ar} & Isolated prompt photon + jet cross-section & ATLAS 7 Photon Jet \\
CMS\_2014\_I1266056 \cite{Chatrchyan:2013mwa} & [Unvalidated] Photon + jets triple differential cross-section & CMS 7 Photon Jet \\
ATLAS\_2012\_I1199269 \cite{Aad:2012tba} & Inclusive diphoton $+ X$ events at $\sqrt{s} = 7$ TeV & ATLAS 7 Diphoton \\

%%####ZZ
ATLAS\_2012\_I1203852 \cite{Aad:2012awa} & Measurement of the $ZZ(*)$ production cross-section in $pp$ collisions at 7 TeV with ATLAS & ATLAS 7 ZZ \\

%%#W/Z+gamma
ATLAS\_2013\_I1217863 \cite{Aad:2013izg}  & W/Z gamma production at 7 TeV & ATLAS W/Z gamma \\
\end{tabular}
\caption{Table of all \rivet routines currently included in the limit-setting scan}\label{tab:Rivet}
\end{center}
\end{table}
...
%\todo{DY to check list is consistent with the final runs; show table of rivet IDs for quick reference here}, 
...
Figure~\ref{fig:ATLASdijet} shows a comparison between the model expectations in the `challenging' scenario and the most sensitive distributions from the ATLAS jet measurements. \todo{MK: Isn't that the intermediate scenario in the figs?}The measured dijet mass distribution is smoothly falling to higher masses, and the presence of a mediator decaying to quarks 
...
\todo{DY: Improve the presentation of this figure}\todo{MK: do we really need a figure here? These are just cross section  numbers, so a table should be sufficient...}
...
right \todo{MK: left region?} region of the map is excluded. In addition, the cross section $\times$ branching ratio for quarks $\rightarrow Z^\prime \rightarrow$ quarks remains large
