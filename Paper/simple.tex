...
measurements at particle colliders, is presented. The method, `Constraints On New Theories Using \rivet' --- \Contur \ --- exploits  \todo{MK: how do you typeset hyphen: -, --, or ---? DG: Has to be the extra long em-dash here, ---, but JB undid my fix lower down. Also should have no surrounding spaces, but more and more papers leave spaces in. Here specifically, I would use () instead or rephrase}
...
and detector simulation via the RECAST~\cite{Cranmer:2010hk} project.}\todo{MK: are footnotes referenced before or after the period? DG: has different meaning: footonote for the sentence: after. Footnote for the word: before.}
...
\todo{MK: do we really need a figure here? These are just cross section  numbers, so a table should be sufficient...DY: put an attemp in but not sure if I even made things clearer?}
