...
%% quantifying the agreement between the null (SM only) hypothesis and the tested hypothesis can be computed. \todo{DY, phrase this carefully. It's the p-value of what exactly, under what assumption? Sth like: probability of observing n events if b-only is true...}considered, comparisons to SM calculations have shown consistency between them and the data. Thus as a starting point, we take the
data as our ``null signal'', and we superpose onto them the contribution from the BSM scenario under consideration.
