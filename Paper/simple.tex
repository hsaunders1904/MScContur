...
a fiducial cross section measurement of $pp \rightarrow l^+l^- + \MET$. This is illustrated in Fig.~\ref{fig:ATLASzz}. 
The production diagrams are the same as the vector-boson-plus-jets case, but in this case the 
...
Finally, althought they were scanned in the limit-setting process, the currently available isolated photon measurements do not contribute signficantly to the exclusion
limits for this model.


\section{Limits}\label{sec:limits}

The sensivities derived from multiple distribiutions such as those discussed in the previous section are combined into `heatmaps' which delineate exclusion 
...
For $\MZP < 2\MDM$, the decay of the mediator to Dark Matter is suppressed. This leads to the diagonal structure across the plots, 
with the sensitivity below the diagonal, in the right portion of the map, coming mainly from the jet measurements.

At low values of the \MZP, the sensivity comes mainly from the $V+$jets signatures. In the challenging and intermediate scenarios, a dip in sensitivity around 
...
enough that the diagonal cutoff in sensivitivty of the jet channels at $\MZP \approx 2\MDM$ is blurred. {\bf illustrate this further with a low \GDM option?}

To the top left region of the diagonal the decay of the mediator to Dark Matter is kinematically allowed, and for the chosen value of $\GDM=1$, will dominate 
over the decay to quarks. Hence the sensivity in the inclusive jet (and $V+$jet) signatures drops. This is the region where a measurement of \MET+jets would be useful 
(and indeed it is where the searches performed using such signatures contribute, see \cite{Kahlhoefer:2015bea}). Current sensitivity dies away at $\MZP \approx 750$~GeV 
in the intermediate and challenginf scenarios. 
...
\centering
    \subfloat[$\GQ=0.25$ and $\GDM = 1$ \label{fig:map025}]{%
      \includegraphics[width=0.5\textwidth]{images/combinedCL_gq025.pdf}
    }    
    \subfloat[$\GQ=0.5$ and $\GDM = 1$\label{fig:map05}]{%
      \includegraphics[width=0.5\textwidth]{images/combinedCL_gq05.pdf}
    }

    \subfloat[$\GQ=0.375$ and $\GDM = 1$\label{fig:map0375}]{%
      \includegraphics[width=0.5\textwidth]{images/combinedCL_gq0375.pdf}
    }
