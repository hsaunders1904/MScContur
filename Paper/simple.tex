...
\subsection{Statistical Method}\label{subsec:statmethod}
...
\begin{figure}[!htb]
\begin{center}
    \subfloat[Simulated signals for a sample of mediator masses. Interpreted as perturbations to the double differential inclusive jet cross section in the most central
rapidity region, binned by dijet mass and rapidity corresponding to the data as measured by ATLAS at 7 TeV~\cite{Aad:2014pua} \label{fig:ATLASdijet}]{%
    ...
    \subfloat[Simulated signals for a sample of mediator masses. Interpreted as perturbations to the double differential dijet cross section in the most central
rapidity region, binned by leading jet $p_{T}$ and rapidity corresponding to the data as measured by CMS at 7 TeV~\cite{Chatrchyan:2014gia} \label{fig:CMSincljet}]{%
    ...
    \subfloat[Simulated signals for a sample of mediator masses. Interpreted as perturbations to the differential cross section of the $W+\geq2$ jet process,
 binned by the mass of the dijet pair corresponding to the data as measured by ATLAS at 7 TeV~\cite{Aad:2014qxa} \label{fig:ATLASzjet}]{%
...
\begin{center}
\ContinuedFloat    
    \subfloat[Simulated signals for a sample of mediator masses. Interpreted as perturbations to the $ZZ\rightarrow ll\nu \nu$ cross section 
corresponding to the data as measured by ATLAS at 7 TeV~\cite{Aad:2012awa} \label{fig:ATLASzz}]{%
      \includegraphics[width=0.45\textwidth]{images/atlaszz/fullrange/ATLASzz.pdf}
    }
        
    \caption{Outputs from \textsc{Rivet} for a sample of potential measurements to be included
 in the limit setting process, figure~\ref{fig:ATLASdijet} taken from the ATLAS dijet measurement, figure~\ref{fig:CMSincljet} from the CMS
inclusive jet measurement~\cite{Chatrchyan:2014gia}, figure~\ref{fig:ATLASzjet} from the ATLAS $Z+$inclusive jet measurement~\cite{Aad:2014qxa} and 
figure~\ref{fig:ATLASzz} from the ATLAS $ZZ$ production cross section. The generated BSM process cross section is constructed as per, and represented alongside, the measured data (left)
 and the ratio of data plus BSM simulation to data is also presented (right). The signals form a 1D parameter space scan in mediator mass for fixed dark matter
 mass and mediator couplings; $m_{\textsc{dm}}=600$ GeV, $g_{q}=0.25$ and $g_{\textsc{dm}} = 1$.}
    \label{fig:rivetout}
\end{center}
\end{figure}
...
\ContinuedFloat
    ...
    section~\ref{subsec:statmethod}. The combination of measurements entering into the confidence level presented here is the maximally
