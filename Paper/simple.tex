...
\begin{align}\label{eq:vector_mediator}
 {\cal L} \supset  \GDM\overline{\psi} \gamma_{\mu}\gamma_5 \psi\,Z'^{\mu} + g_{\rm SM}\sum_{q} \bar q \gamma_{\mu} q Z'^{\mu} \,,
\end{align}
%
...
has only four free parameters, two couplings and two masses: \GDM, $g_{\rm SM} \equiv \GQ, M_\psi \equiv \MDM$, and \MZP. The width of the mediator, 
...
perturbative regime, \GDM, $\GQ \lesssim {\cal O}(1)$, so that our predictions for DM and mediator production at the LHC 
are well-defined. 
...
model re-interpretations of LHC searches as presented in, e.g.,  Refs.~\cite{Kahlhoefer:2015bea, Heisig:2015ira}. 

To simulate the experimental signature for our model, we have generated events using \textsc{FeynRules}~\cite{Alloul:2013bka}... 
{\bf MK: please complete the description of the tool chain...}
...
have begun to appear\cite{Aad:2015hna,Khachatryan:2016gxp}. These could are potentially very powerful in excluding some models, but will in principle 
...
\begin{itemize}
\item $\tilde{b}$, a variable background count, at face value this is equal to $b$, however it is marked differently as it can be varied in order to use 
Monte Carlo methods to extract the sensitivity if desired.
\item $\sigma_{b}$, Uncertainty in $b$ count taken from \rivet as 1 $\sigma$ error on a Gaussian (uncertainties taken as the combination of statistical and 
systematics uncertaintes in quadrature. Typically the systematic uncertainty dominates).
\end{itemize}
\end{itemize}
...
the couplings. These coupling choices correspond to (i) an `optimistic' scenario $\GQ = 0.5, \GDM = 1$ - strong signals, 
close to the edge of exclusion already, (ii) a `challenging' scenario $\GQ = 0.25, \GDM = 1$ - low couplings, hard to exclude, 
and (iii) an `intermediate' scenario $\GQ = 0.375, \GDM = 1$, between the two.
...
Figure~\ref{fig:ATLASdijet} shows a comparison between the model expectations and the most sensitive distributions from the ATLAS jet measurements. The measured 
...
\caption{Outputs from \rivet for a measurement included in the limit setting process. Simulated signals for a sample of mediator masses, interpreted as perturbations to the double differential dijet cross section in the most central rapidity region, binned by leading jet \pt and rapidity corresponding to the data as measured by CMS at 7 TeV~\cite{Chatrchyan:2014gia}. The left hand plot compares the measured cross section to the model expectation, and the right hand plot shows the perturbation 
in the ration compared to the relative uncertainty in the measurement. The signals form a 1D parameter space scan in mediator mass for fixed dark matter mass and mediator couplings; $\MDM=600$ GeV, $\GQ=0.25$ and $\GDM = 1$.}
...
\caption{Outputs from \rivet for a potential measurement to be included in the limit setting process. Simulated signals for a sample of mediator masses, interpreted as perturbations to the differential cross section of the $W+\geq2$ jet process, binned by the mass of the dijet pair corresponding to the data as measured by ATLAS at 7 TeV~\cite{Aad:2014qxa}. The signals form a 1D parameter space scan in mediator mass for fixed dark matter mass and mediator couplings; $\MDM=600$ GeV, $\GQ=0.25$ and $\GDM = 1$.}
\label{fig:ATLASwjet}
\end{figure}

Moving on to the 'Electroweak' final states discussed in Section~\ref{sec:measurements}, Figure~\ref{fig:ATLASwjet} illustrates the sensivity of vector-boson-plus-jet
measurements to this model, in this case the dijet mass differential cross section in $W$-boson-plus-jet events. Strictly speaking, the measurement is made for
events with a single charged lepton, \MET, and jets, interpreted as $W$+jets in the SM. In the BSM model considered here, \MET could also arise from the Dark Matter
candidate. However, inspections shows that the sensitivity is arise from genuine $W$ bosons produced in association with the mediator - not a signature typcially
considered in constraints on this class of model.
...
sample of mediator masses, Interpreted as perturbations to the $ZZ\rightarrow ll\nu \nu$ cross section 
...
For $\MZP < 2\MDM$, the decay to Dark Matter is suppressed and so the sensitivity comes mainly from the jet measurements.

\begin{figure}[p]
\centering
    \subfloat[Mass plane scan for fixed mediator couplings; $g_{q}=0.25$ and $g_{\textsc{dm}} = 1$ \label{fig:map025}]{%
      \includegraphics[width=0.75\textwidth]{images/combinedCL_gq025.pdf}
    }
    
    \subfloat[Mass plane scan for fixed mediator couplings; $g_{q}=0.5$ and $g_{\textsc{dm}} = 1$\label{fig:map05}]{%
      \includegraphics[width=0.75\textwidth]{images/combinedCL_gq05.pdf}
    }
...
\centering
\ContinuedFloat
    \subfloat[Mass plane scan for fixed mediator couplings; $g_{q}=0.375$ and $g_{\textsc{dm}} = 1$\label{fig:map0375}]{%
      \includegraphics[width=0.75\textwidth]{images/combinedCL_gq0375.pdf}
    }
