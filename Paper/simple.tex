...
measurement from ATLAS~\cite{Aad:2015nda} \todo{DY: I haven't included 4jet 8tev in what we currently display, it had pretty much no sensitivity to the model and was a whole additional set of event generation. Cut this out? I (JMB) think since we tried it and checked, it's best to leave this wording in.}.
\item
...
photon-plus-jet~\cite{ATLAS:2012ar} measurements \todo{DY: I am unsure on whether there is a guarantee of no statistical overlap of the photon + jet and inclusive photon, I mark as separate catagories for now but is this safe? has almost no impact either way as one is 37pb-1 so gives no stat power anyway. JMB: there will be overlap if they are measured in the same sample.}, where available. \todo{DY: Ref for unvalidated CMS gamma + jet we include?}
\item
