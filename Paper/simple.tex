...
\it $^2$IPPP, Department of Physics,\\\it Durham University, DH1 3LE, UK\\[1mm] \it $^3$Institute for Theoretical Particle Physics and Cosmology, \\ \it RWTH Aachen University, 52056 Aachen, Germany}
...
\begin{abstract}
A new method providing general consistency constraints for Beyond-the-Standard-Model (BSM) theories, using 
...
%
\begin{flushright}IPPP-16-52, MCNET-16-21, TTK-16-22\end{flushright}
\end{abstract}
...
a BSM configuration is created for Herwig~7.0.1~\cite{Bellm:2015jjp,Bahr:2008pv}. For each parameter point in the scan grid, events were generated in Herwig and 
...
from neutrinos found in the simulated event record, rather than as the negative of the visible particles in the event. These 
...
\breaker
ATLAS 7 $W$ jets & ATLAS\_2014\_I1319490 \cite{Aad:2013ysa} & $W$ + jets \\
%%####Gamma(+jets)
\breaker
...
energies (see Table~\ref{tab:Rivet}).
...
\item $n$, the observed total count (in the absence of a separate background simulation, this is taken to be $b$)
...
\end{itemize}
\item An additional Poisson term describing the error on the BSM signal count, this is taken as a Monte Carlo error on the simulated count. The additional counting parameter $k$, has an expectation value of $s$.
\end{itemize}
%
Note that in the absence of a separate simulation of the background, we take $n=m=b$ and $k=s$.
%
...
Motivated by the approach of Ref.~\cite{Cowan:2010js}, we use the so called Asimov data set to build an approximate distribution of the considered test statistic. 
...
the methodology presented here: the combination is the product of the likelihoods of all the selected counts in a new combined likelihood function. 
The rest of the process is unchanged when considering the combination. Consequently, the 95\% CL contour displayed in Figure~\ref{fig:cont} displays  the regions in parameter space where the observed counts $n_{i}$ across all the considered bins, yield a probability of less than 5\% of being compatible with the full signal strength hypothesis, and consequently the model is considered excluded by the data taken.
...
7 TeV~\cite{Aad:2014pua}. The upper plot compares the measured cross section to the model expectation, and the lower hand plot shows the perturbation 
...
$\GQ=0.25$ and $\GDM = 1$. The corresponding exclusion limits are also given.}
\label{fig:ATLASdijet}
\end{figure}

Figure~\ref{fig:ATLASdijet} shows a comparison between the model expectations in the `intermediate' scenario and the most sensitive distributions from the ATLAS jet measurements. 
The measured dijet mass distribution is smoothly falling to higher masses, and the presence of a mediator decaying to quarks 
...
CMS at 7 TeV~\cite{Chatrchyan:2014gia}. The upper plot compares the measured cross section to the model expectation, and the lower plot shows the perturbation 
...
corresponding to the data as measured by ATLAS at 7 TeV~\cite{Aad:2012awa}. The signals form a 1D parameter space scan in 
...
At low values of \MZP the sensitivity comes mainly from the $V+$jets signatures. In the challenging and intermediate scenarios, a dip in sensitivity around 
...
\caption{Contours in the \MZP and \MDM plane for the considered values of \GDM and \GQ, indicating the excluded region at 95\% confidence level. The triangular shaded
area is the region in which perturbative unitary is violated by the model.}\label{fig:cont}
\end{figure}
%
...
The other channels extend the sensitivity, and this will also improve as more measurements are incorporated.
...
a method to efficiently scan existing particle-level measurements from the LHC, implemented in \rivet, to derive limits on new physics. 
