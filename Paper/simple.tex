...
\it $^2$IPPP, Department of Physics,\\\it Durham University, DH1 3LE, United Kingdom\\[1mm] \it $^3$Institute for Theoretical Particle Physics and Cosmology, \\ \it RWTH Aachen University, D-52056 Aachen, Germany}
...
In this paper we make extensive use of these capabilities within Herwig~\cite{Bahr:2008pv} \todo{DG: Herwig 7 cite}.
\item
...
searches can be extended \textbf{todo: DG: what does that mean?} to new models by accessing archived versions of the original analysis code, ideally using a standardised 
interface such as that provided by the RECAST~\cite{Cranmer:2010hk} project.}
Any theory seeking
to explain a new signature or anomaly in the data may have consequences \todo{DG: clarify} for other final states which should be checked 
against data this way. If no BSM physics emerges, a model-independent and systematic approach becomes mandatory.\todo{DG: clarify}

In this paper, we first motivate and describe the 
simplified model we have chosen as an initial demonstration, and its simulation using Feynrules and Herwig. 
...
\todo{MK: please complete the description of the tool chain}
...
explicity assumed to be the same as neutrino transverse energy. In BSM physics, missing energy can also 
...
we define a prodecure to combine exclusion limits from different measured distributions.
...
Taking each bin of each distribution considered as a separate statistic to be tested, a likelihood function can be constructed as follows, \todo{DG: fix as discussed, breaking into multiple steps}
\begin{align}
...
{\bf here we should have some discussion of the theory constraints, other experiment constraints, and cosmological context.}
...
They also thank Josh McFayden for useful discussions, and STFC for financial support.
