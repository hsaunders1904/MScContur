...
\label{sec:intro}
\todo{DG to confirm MCnet preprint number on submission day at \url{http://montecarlonet.org/index.php?p=Publications/2016}}
...
In this paper we make extensive use of these capabilities within Herwig~\cite{Bahr:2008pv} \todo{DG to find Herwig 7 cite}.
\item
...
new physics models or narrow down the corresponding model parameter space.\todo{JB to clarify}

In this paper, we first motivate and describe the 
simplified model we have chosen as an initial demonstration, and its simulation using Feynrules~\cite{Alloul:2013bka} and Herwig~\cite{}. \textbf{DG sorts this out, mention tools later. MK added citation here because its the first mention of Feynrules. Maybe  better mention it before and drop citation here....]}
...
$\Gamma_{Z'}$, is determined by these four parameters. \todo{ALL: we might have to adjust this depending on whether we show results with pr without the heavy flavor contributions.}
...
\todo{DG will complete the description of the tool chain}. Higher-order QCD corrections have been calculated for this class of dark matter simplified models 
...
a particular BSM theory in great detail, we will use leading-order signal cross section predictions as provided by Herwig. \todo{DG answers if are the LO cross sections indeed generated by Herwig, or are we using Madgraph? DG to sort out} 
...
routines were modified as a part of this work, and are all fixed in release 2.5.X.} \todo{JB: write clearer distinction between what experimental data analyses should do / what rivet MC analyses should do}
...
measurement from ATLAS~\cite{Aad:2015nda} \todo{DY: I haven't included 4jet 8tev in what we currently display, it had pretty much no sensitivity to the model and was a whole additional set of event generation. Cut this out?}.
\item
...
and CMS~\cite{Khachatryan:2014uva},\todo{DY: ref for CMS Z+jets needed} are used.
...
photon-plus-jet~\cite{ATLAS:2012ar} measurements \todo{DY: I am unsure on whether there is a guarantee of no statistical overlap of the photon + jet and inclusive photon, I mark as separate catagories for now but is this safe? has almost no impact either way as one is 37pb-1 so gives no stat power anyway}, where available. \todo{DY: Ref for unvalidated CMS gamma + jet we include?}
\item
...
implemented in \rivet. %\todo{DG: DG motivate why we're not hand-writing analyses}
...
Starting with the measurements discussed in Section~\ref{sec:measurements} \todo{DY to check list is consistent with the final runs; show table of rivet IDs for quick reference here}, 
...
Hence the $p$-value quantifying the agreement between the null (SM only) hypothesis and the tested hypothesis can be computed. \todo{DY, phrase this carefully. It's the p-value of what exactly, under what assumption? Sth like: probability of observing n events if b-only is true...}
...
%\todo{MK: I prefer larger figures, see below, but leave this to you to decide. JMB: I agree and have implemented it.}
\todo{DY provide CL values for the 4 options shown, clarify rivet plot titles}
...
\todo{DY: Improve the presentation of this figure}
The production diagrams are the same as the $V$-jets case, but in this case the 
mediator decays to dark matter rather than back to quarks. \todo{JB to sort this out... MK: maybe we should show a couple of example Feynman diagrams at some point? JMB: I think that would be very good 
 - can someone make them? I'd say the s-channel production and decays to quarks and to DM, and the production with a gauge boson plus decay too? DY: Added some that I had from my X-mas talk feel free to use or not}
...
\begin{figure}[p]
\centering
\includegraphics[width=0.5\textwidth]{images/feyndia/vec_dm_born.pdf}
\\
\includegraphics[width=0.5\textwidth]{images/feyndia/vec_dm_g.pdf}
\\
\includegraphics[width=0.5\textwidth]{images/feyndia/vec_dm_weak.pdf}
\caption{Rough feynman diagrams}\label{fig:feyndia}
\end{figure}
...
\todo{MK writes some discussion of the theory constraints, other experiment constraints, and cosmological context.}
...
Rivet ID (Inspire) & Rivet description & \Contur Category \\
\hline
ATLAS\_2014\_I1325553 & Measurement of the inclusive jet cross-section at 7 TeV & ATLAS 7 Jets \\
ATLAS\_2014\_I1268975 & High-mass dijet cross section & ATLAS 7 Jets \\
ATLAS\_2014\_I1326641 & 3-jet cross section with 7 TeV data & ATLAS 7 Jets \\
ATLAS\_2014\_I1307243 & Measurements of jet vetoes and azimuthal decorrelations in dijet events produced in pp collisions at $\sqrt{s}$ = 7 TeV using the ATLAS detector & ATLAS 7 Jets \\
CMS\_2014\_I1298810 & Ratios of jet pT spectra, which relate to the ratios of inclusive, differential jet cross sections & CMS 7 Jets \\

%%####ZJets
ATLAS\_2013\_I1230812 & $Z$ + jets in $pp$ at 7 TeV & ATLAS 7  ZJets \\
CMS\_2015\_I1310737 & Jet multiplicity and differential cross-sections of $Z$+jets events in $pp$ at $\sqrt{s} = 7$ TeV & CMS 7 Z Jets \\

%%####WJets
CMS\_2014\_I1303894 & Differential cross-section of $W$ bosons + jets in $pp$ collisions at $\sqrt{s}=7$ TeV & CMS 7 W Jets \\
ATLAS\_2014\_I1319490 & W + jets & ATLAS W jets \\

%%####Gamma(+jets)
ATLAS\_2013\_I1263495 & Inclusive isolated prompt photon analysis with 2011 LHC data & ATLAS 7 Gamma \\
ATLAS\_2012\_I1199269 & Inclusive diphoton $+ X$ events at $\sqrt{s} = 7$ TeV & ATLAS 7 gamma gamma \\
ATLAS\_2012\_I1093738 & Isolated prompt photon + jet cross-section & ATLAS 7 Gamma Jet \\
CMS\_2014\_I1266056 & [Unvalidated] Photon + jets triple differential cross-section & CMS 7 Gamma jet \\

%%####ZZ
ATLAS\_2012\_I1203852 & Measurement of the $ZZ(*)$ production cross-section in $pp$ collisions at 7 TeV with ATLAS & ATLAS 7 ZZ \\

%%#W/Z+gamma
ATLAS\_2013\_I1217863 & W/Z gamma production at 7 TeV & ATLAS W/Z gamma \\
\end{tabular}
\caption{Table of all rivet routines included in method}\label{tab:Rivet}
\end{center}
\end{table}
