...
highly-correlated systematic uncertainties, such as those on the integrated luminosity, or the jet energy scale.

One feature noted in several of these routines is the fact in a number of cases, missing transverse energy (\MET) is 
explicity assumed to be the same as neutrino transverse energy. In fact, in a BSM study, missing energy can also 
arise from other sources (for example, Dark Matter) and so it is important that the result is treated in such a 
way that this sensitivity is correctly estimated. The measurements are typically corrected back to total \MET, 
or to the assumed neutrino $p_T$, by the experimental analysis, using a simulated SM event sample. 
This involves an extrapolation into the forward 
region where transverse energy is unmeasured; however, unless a BSM particle enters this region, the error made is 
negligible, so as long fiducial is is made on BSM particles counting toward \MET, such analyses can be 
...
routines were modified in release 2.4.0 as a part of this work.}
...
\item
Electroweak: events with leptons, with or without missing energy or photons.
\item
Missing energy, possibly with jets but no leptons or photons. There are no fully-correct particle-level distributions available. However the ATLAS search for supersymmetry, with jets and missing energy~\cite{Aad:2012fqa} does have a rivet routine and can be used approximately (I hope)
\item
