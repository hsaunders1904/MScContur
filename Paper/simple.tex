...
rapidity region, binned by by leading jet $p_{T}$ and rapidity corresponding to the data as measured by CMS at 7 TeV~\cite{Chatrchyan:2014gia} \label{fig:CMSincljet}]{%
    ...
    \caption{Outputs from \textsc{Rivet} for a sample of two potential plots to be included
 in the limit setting process, figure~\ref{fig:ATLASdijet} taken from the ATLAS dijet measurement and figure~\ref{fig:CMSincljet} from the CMS
inclusive jet measurement~\cite{Chatrchyan:2014gia}. The generated BSM process cross section is constructed as per, and represented alongside, the measured data (left)
